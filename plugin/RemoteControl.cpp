...
namespace WPEFramework {
    ...
    namespace Plugin {
        ...
        void RemoteControl::iarmEventHandler(const char *owner, IARM_EventId_t eventId, void *data, size_t len)
        {
            ...
            {
                LOGERR("ERROR - unexpected event: owner %s, eventId: %d, data: %p, size: %zu.",
            ...
            switch(eventId) {
                ...
                case CTRLM_RCU_IARM_EVENT_FIRMWARE_UPDATE_PROGRESS:
                    ...
                    break;
                default:
                    LOGERR("ERROR - unexpected ctrlm event: eventId: %d, data: %p, size: %zu.",
        ...
        uint32_t RemoteControl::stopPairing(const JsonObject& parameters, JsonObject& response)
        {
            ...
            if (bSuccess)
                LOGINFO("STOP PAIRING call SUCCESS! %s");
            else
